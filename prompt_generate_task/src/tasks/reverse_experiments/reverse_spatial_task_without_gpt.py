...
REVERSE_TEMPLATE_DIR = os.path.join(REVERSE_DATA_DIR, "spatial_template")
...
def generate_alt_examples(nameA: str, nameB: str, templates: List[str], a2b: bool) -> List[Dict[str, str]]:
    ...
    for template in templates:
        ...
        # 你可以选择 split point 处理 prompt/completion，也可以全部放进 prompt
        # 这里用简单策略：<nameB> 之前是 prompt，之后是 completion
        if a2b:
...
class ReverseTask:
    ...
    def save(self, directory: str):
        ...
        # For completions of names, we want only the first and last name
        a2b_reverse_prompts_test = flatten(
            [[shorten_completion(e) for e in example.b2a_test_examples] for example in self.a2b_examples]
        )
        b2a_reverse_prompts_test = flatten([example.a2b_test_examples for example in self.b2a_examples])
        validation_prompts = [self.to_validation_prompt(prompt) for prompt in a2b_reverse_prompts_test]
        ...
        names = [
            ...
            (a2b_reverse_prompts_test, "a2b_reverse_prompts_test"),
            (b2a_reverse_prompts_test, "b2a_reverse_prompts_test"),
